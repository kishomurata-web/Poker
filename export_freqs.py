...
def rows_for_file(path):
    ...
    with open(path, encoding="utf-8") as fh:
        for raw in fh:
            ...
            # A resumed run can append a second copy of a card it already had.
            ...
            if freqs is None:
                continue
            pot = rec.get("pot") or 0
            for code, f in zip(rec["actions"], freqs):
                frac = action_frac(code, pot)
                yield (pair, line, node, board, card, "%g" % pot, "%.1f" % combos,
                       code, "" if frac is None else "%.4f" % frac, "%.6f" % f)
...
def selftest():
    ...
    with open(turn, "w", encoding="utf-8") as fh:
        for card, mix in (("2c", (3000, 7000)), ("Kd", (9000, 1000))):
            fh.write(json.dumps({
                "card": card, "pot": 12.2, "actions": ["X", "R4"],
    ...
    check("an all-in has no pot fraction", action_frac("RAI", 6.1), None)
